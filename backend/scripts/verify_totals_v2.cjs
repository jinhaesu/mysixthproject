...
async function main() {
  ...
  console.log(`V2 sum: ${v2Sum.toLocaleString()} (${v2Map.size} rows)`);
  ...
  // === Replicate API exactly ===
  // 1. Get all confirmed_attendance for the month
  ...
  // 3. Group by employee (phone canonical)
  ...
  // 5. Get salaries (same JOIN as backend)
  ...
  `, [yearMonth, monthStart])).rows;

  let sysSum = 0;
  let sysSumNonZero = 0;
  let sysCountNonZero = 0;
  const diffs = [];
  for (const sal of salaries) {
    ...
    const adj = parseFloat(sal.adjustment_amount) || 0;
    const gross = basePay + mealAllowance + parseFloat(sal.bonus) + parseFloat(sal.position_allowance) + parseFloat(sal.other_allowance) + otPay + holPay + adj;
    sysSum += gross;
    if (gross > 0) {
      sysSumNonZero += gross;
      sysCountNonZero++;
    ...
    if (v2Map.has(sal.name)) {
      ...
      if (Math.abs(gross - v2g) > 1) {
        diffs.push({ id: sal.employee_id, name: sal.name, sys: gross, v2: v2g, diff: gross - v2g });
      }
    }
  }
  console.log(`\nSystem sum (all employees): ${sysSum.toLocaleString()}`);
  console.log(`System sum (gross > 0): ${sysSumNonZero.toLocaleString()} (${sysCountNonZero} rows)`);
  console.log(`Diff vs v2: ${(sysSum - v2Sum).toLocaleString()}`);
  console.log(`\nDiffs (system != v2): ${diffs.length}`);
  for (const d of diffs.slice(0, 30)) {
    console.log(`  id=${d.id} ${d.name}: sys=${d.sys.toLocaleString()} v2=${d.v2.toLocaleString()} diff=${d.diff.toLocaleString()}`);
  }
  await pool.end();
}
