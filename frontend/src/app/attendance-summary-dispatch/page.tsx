  ...
  const handleConfirmRows = async () => {
    ...
    try {
      ...
      for (const key of Array.from(checkedRows)) {
        ...
        const h = calcHoursFromTimes(clockIn, clockOut, breakH);
        records.push({ employee_type: emp.type || '파견', employee_name: emp.name, employee_phone: emp.phone, department: emp.department || '', date, confirmed_clock_in: clockIn, confirmed_clock_out: clockOut, source, regular_hours: h.regular, overtime_hours: h.overtime, break_hours: breakH, year_month: `${year}-${String(month).padStart(2, '0')}` });
      }
