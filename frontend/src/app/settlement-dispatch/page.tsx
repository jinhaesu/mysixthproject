...
export default function SettlementDispatchPage() {
  ...
  const calcEmp = (r: any, idx: number) => {
    ...
    // 어긋나는 이슈로 인해 계산식을 어제 이전 상태로 되돌림.
    ...
    return { rate, basePay, overtimePay, holidayPay, nightPay, whPay, grossPay, meal, net, np, hi, ia, ei, ltc, ins, fee, bv, vat, total: bv + vat };
  };
  ...
  const totals: any = {};
  const numKeys = ['work_days','regular_hours','overtime_hours','night_hours','weekly_holiday_hours','basePay','overtimePay','holidayPay','nightPay','whPay','grossPay','meal','net','np','hi','ia','ei','ltc','ins','fee','bv','vat','total'];
