...
function findDuplicates(records: AttendanceRecord[]): DuplicateEntry[] {
  ...
  for (const record of records) {
    ...
    if (!dateNameMap.has(key)) dateNameMap.set(key, []);
...
function findBasicWarnings(records: AttendanceRecord[]): WarningEntry[] {
  ...
  for (const record of records) {
    ...
    if (!record.clock_in || !record.clock_out) {
      warnings.push({
        type: 'missing_data', severity: 'medium',
...
export async function analyzeAttendance(records: AttendanceRecord[]): Promise<AnalysisResult> {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    ...
    if (duplicates.length > 0) summaryParts.push(`${duplicates.length}건의 중복 기록이 발견되었습니다.`);
    if (basicWarnings.length > 0) summaryParts.push(`${basicWarnings.length}건의 주의사항이 발견되었습니다.`);
    if (duplicates.length === 0 && basicWarnings.length === 0) summaryParts.push('특별한 이상사항은 발견되지 않았습니다.');
    return { duplicates, warnings: basicWarnings, summary: summaryParts.join(' ') };
  }
