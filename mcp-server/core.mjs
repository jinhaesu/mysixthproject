/**
 * MCP Core - 자동 업데이트 대상 파일
 * proxy.mjs가 시작 시 GitHub에서 최신 버전을 다운로드
 */
...
async function verifyPw(pw) {
  try { return (await a("/api/regular/verify-password","POST",{password:pw})).verified; } catch { return false; }
}
...
s.tool("get_monthly_summary","월별 통계",{year_month:z.string(),type:z.string().describe("regular/dispatch")},async({year_month,type})=>{
  const d=await a(`/api/regular/confirmed-list?year_month=${year_month}`);
  const f=(d||[]).filter(e=>type==='regular'?e.type==='정규직':e.type!=='정규직');
  if(!f.length)return t("없음");
  const tt=f.reduce((a,e)=>({w:a.w+1,d:a.d+e.days,r:a.r+e.regular_hours,o:a.o+e.overtime_hours,n:a.n+e.night_hours}),{w:0,d:0,r:0,o:0,n:0});
  return t(`${year_month} ${tt.w}명 ${tt.d}일 기본${tt.r.toFixed(0)}h 연장${tt.o.toFixed(0)}h 야간${tt.n.toFixed(0)}h`);
});
...
// ===== 비밀번호 필요 =====
s.tool("get_settlement","정산 조회 🔒",{year_month:z.string(),type:z.string().optional().describe("dispatch/alba"),password:z.string().describe("비밀번호")},async({year_month,type,password})=>{
...
s.tool("get_salary_settings","기본급 조회 🔒",{password:z.string().describe("비밀번호")},async({password})=>{
  if(!await verifyPw(password))return t("❌ 비밀번호 불일치");
...
s.tool("get_payroll_calc","급여 계산 🔒",{year_month:z.string(),password:z.string().describe("비밀번호")},async({year_month,password})=>{
  if(!await verifyPw(password))return t("❌ 비밀번호 불일치");
  const d=await a(`/api/regular/payroll-calc?year_month=${year_month}`);
  if(!d?.results?.length)return t(`${year_month} 없음`);
  return t(`${d.results.length}명:\n${d.results.map(r=>`${r.name} ${r.work_days}일 기본급${r.base_pay||0} 연장${r.overtime_hours?.toFixed(1)||0}h`).join('\n')}`);
});

s.tool("update_salary","기본급 수정 🔒",{employee_name:z.string(),base_pay:z.number().optional(),meal_allowance:z.number().optional(),password:z.string().describe("비밀번호")},async({employee_name,base_pay,meal_allowance,password})=>{
  if(!await verifyPw(password))return t("❌ 비밀번호 불일치");
  const emps=(await a("/api/regular/employees?limit=500")).employees||[];
